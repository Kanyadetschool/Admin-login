...
//                 name: student.StudentFullName,
//                 issues: issues,
//                 student: student // Keep reference for additional filtering
//             };
//         }
//     });
...
//     // Get unique grades and issues for filter options
...
//                     const coloredIssues = student.issues.map(issue => 
//                         `<span style="color: ${issueColors[issue] || '#dc3545'}; font-weight: bold;">${issue}</span>`
//                     ).join(', ');
                    
//                     return `
//                         <li style="border: 1px solid #dee2e6; border-radius: 6px; padding: 12px; margin-bottom: 8px; background: #fff;">
//                             <div style="display: flex; justify-content: space-between; align-items: center;">
//                                 <div>
//                                     <span style="background: #0d6efd; color: white; padding: 2px 8px; border-radius: 12px; font-size: 12px; margin-right: 8px;">
//                                         ${student.grade}
//                                     </span>
//                                     <strong>${student.name}</strong>
//                                 </div>
//                                 <span style="background: #f8f9fa; padding: 4px 8px; border-radius: 4px; font-size: 12px; color: #6c757d;">
//                                     ${student.issues.length} issue${student.issues.length > 1 ? 's' : ''}
//                                 </span>
//                             </div>
//                             <div style="margin-top: 8px; font-size: 14px;">
//                                 Missing: ${coloredIssues}
//                             </div>
//                         </li>
//                     `;
//                 }).join('');
...
function showDataQualityModal(allIssues) {
    let filteredIssues = [...allIssues];
    exportData = [...allIssues]; // Store for export
    ...
    <div style="text-align: left;">
        <!-- Add the Quick Edit button at the top -->
        <div style="margin-bottom: 15px; text-align: center;">
            <div id="fastEditButton" onclick="handleFastEditFromModal()" style="
                background: linear-gradient(135deg, #ff0e0eff 0%, #f3780cff 100%);
                color: white;
                padding: 12px 20px;
                border-radius: 8px;
                cursor: pointer;
        ...
        <div style="background: linear-gradient(135deg, #fff 0%, #ffffff 100%); padding: 15px; border-radius: 8px; margin-bottom: 15px; color: white;">
            <!-- ... existing content ... -->
        </div>
        <!-- ... rest of modal ... -->
    </div>
        ...
        <div style="text-align: left;">
            ...
            <div style="background: transparent;box-shadow: 0 0 20px 1px rgba(0, 0, 0, 0.1); padding: 15px; border-radius: 8px; margin-bottom: 15px; color: white;">
                <div style="display: flex; flex-wrap: wrap; gap: 8px; justify-content: center;">
                    <button id="showAnalytics" class="advanced-btn" style=" background: linear-gradient(45deg, #2e374b, #ff1cac); color: white; border: 1px solid rgba(255,255,255,0.3); hover:none padding: 8px 12px; border-radius: 20px; cursor: pointer; transition: all 0.3s;">
                        📊 Analytics Dashboard
                    </button>
    ...
    Swal.fire({
        ...
        didOpen: () => {
             ...
             // Add event listener inside the modal
        ...
        if (fastEditBtn) {
            fastEditBtn.addEventListener('click', () => {
                ...
                if (issuesList.length > 0) {
                    Swal.close(); // Close current modal first
            ...
            // Function to render quick stats
            function renderQuickStats() {
                ...
                const topIssue = Object.keys(issueFrequency).reduce((a, b) => issueFrequency[a] > issueFrequency[b] ? a : b, '');

                quickStats.innerHTML = `
                    <div style="background: #e3f2fd; padding: 12px; border-radius: 6px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #1976d2;">${totalStudents}</div>
                    ...
                    <div style="background: #ffebee; padding: 12px; border-radius: 6px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #d32f2f;">${criticalCount}</div>
                        <div style="font-size: 12px; color: #666;">Critical (5+)</div>
                    </div>
                    <div style="background: #fff3e0; padding: 12px; border-radius: 6px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #f57c00;">${highCount}</div>
                    ...
                    <div style="background: #f3e5f5; padding: 12px; border-radius: 6px; text-align: center;">
                        <div style="font-size: 24px; font-weight: bold; color: #7b1fa2;">${avgIssuesPerStudent}</div>
                        <div style="font-size: 12px; color: #666;">Avg Issues</div>
                    </div>
                    <div style="background: #e8f5e8; padding: 12px; border-radius: 6px; text-align: center;">
                        <div style="font-size: 16px; font-weight: bold; color: #2e7d32;">${topIssue}</div>
                        <div style="font-size: 12px; color: #666;">Top Issue</div>
                    </div>
                `;
            }

            // Function to get severity class and level
            ...
            function renderResults() {
                ...
                const htmlContent = filteredIssues.map((student, index) => {
                    const issueColors = {
                        ...
                        'Father': '#d63384', 'Mother': '#6f42c1', 'Email': '#ffc107'
                    };
                    
                    const severity = getSeverity(student.issues.length);
                    const coloredIssues = student.issues.map(issue => 
                        `<span class="issue-chip" style="background-color: ${issueColors[issue] || '#dc3545'};">${issue}</span>`
                    ).join(' ');
                    ...
                    return `
                        <li class="student-card ${severity.class}" data-student-id="${student.grade}-${student.name}">
                            <div style="display: flex; justify-content: between; align-items: flex-start; margin-bottom: 10px;">
                                <div style="flex: 1;">
                                    <div style="display: flex; align-items: center; margin-bottom: 5px;">
                                        <span style="background: #0d6efd; color: white; padding: 3px 10px; border-radius: 15px; font-size: 12px; margin-right: 10px;">
                                            ${student.grade}
                                        </span>
                                ...
                                <div style="text-align: right;">
                                    <button class="mark-resolved" data-student="${student.grade}-${student.name}" 
                                            style="background: #28a745; color: white; border: none; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 11px; margin-bottom: 5px;">
                                        ✓ Mark Resolved
                                    </button>
                                    <div style="font-size: 11px; color: #999;">ID: ${index + 1}</div>
                                </div>
                            </div>
                            <div style="margin-top: 10px;">
                                <strong style="font-size: 13px; color: #333;">Missing Data:</strong><br>
                                ${coloredIssues}
                            </div>
                        </li>
                    `;
            ...
            function applyFilters() {
                ...
                filteredIssues = allIssues.filter(student => {
                    // Search filter - expanded to include issues
                    const matchesSearch = !searchTerm || 
                        student.name.toLowerCase().includes(searchTerm) ||
                        student.grade.toLowerCase().includes(searchTerm) ||
            ...
            function markStudentResolved(studentId) {
                // Store resolved students (in a real app, this would persist)
...
function showAnalyticsDashboard() {
    ...
    if (result.isConfirmed) {
        // ✅ Confirm clicked → Back to Data Quality
        showDataQualityModal(window.currentDataQualityIssues);
    } else if (result.dismiss === Swal.DismissReason.cancel) {
        // ❌ Cancel clicked → Just close (do nothing)
    }
});
}
...
function showTrendsInsights() {
    ...
    Swal.fire({
        ...
        `,
        width: '900px',
        confirmButtonText: 'Generate Action Plan',
        showCancelButton: true,
