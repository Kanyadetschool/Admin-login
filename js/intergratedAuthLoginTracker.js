...
async function checkSessionValidity(email) {
    ...
    try {
        ...
        // Check if session has been revoked (admin can set revokeSessionsAfter timestamp)
        const revokeAfter = userData.revokeSessionsAfter;
        if (revokeAfter) {
            const revokeTimestamp = revokeAfter.toMillis ? revokeAfter.toMillis() : revokeAfter;
            const codeVerifiedTime = parseInt(sessionStorage.getItem('codeVerifiedTime_' + email) || '0');
            
            if (codeVerifiedTime < revokeTimestamp) {
                AUTH_LOG.error('Session revoked by administrator');
                return { valid: false, reason: 'Session revoked. Please login again.' };
            }
        }
        
        // Check if access code has changed (requires re-verification)
        const storedCode = userData.accessCode || userData.access_code;
        const lastVerifiedCode = sessionStorage.getItem('verifiedCode_' + email);
        
        if (storedCode && lastVerifiedCode && storedCode.toString() !== lastVerifiedCode) {
            AUTH_LOG.error('Access code has changed');
            return { valid: false, reason: 'Access code changed. Please verify again.' };
        }
...
function startSessionMonitoring(email) {
    AUTH_LOG.log('Starting session monitoring for:', email);
...
function stopSessionMonitoring() {
    if (sessionCheckInterval) {
        clearInterval(sessionCheckInterval);
        sessionCheckInterval = null;
        AUTH_LOG.log('Session monitoring stopped');
    }
}
...
async function logLoginActivity(email, status, errorMessage = null) {
    try {
        ...
        // Try to get IP and location (optional, won't block if fails)
        try {
...
async function isEmailAllowed(email) {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            // Check if approved
            if (userData.approved !== true) {
                AUTH_LOG.log('User exists but not approved');
                return false;
            }
...
async function verifyAccessCode(email, code) {
    ...
    try {
        ...
        // Handle both accessCode and access_code field names
        // Convert both to strings for comparison
        const storedCode = userData.accessCode || userData.access_code;
        const inputCode = code.toString().trim();
        const storedCodeStr = storedCode ? storedCode.toString().trim() : '';
        
        AUTH_LOG.log('Stored code:', storedCodeStr);
        AUTH_LOG.log('Input code:', inputCode);
        AUTH_LOG.log('Code match:', storedCodeStr === inputCode);
        
        if (storedCode && storedCodeStr === inputCode) {
            // Only try to update if we have write permissions, but don't fail if we can't
            try {
                await docRef.update({
                ...
                AUTH_LOG.log('Could not update timestamp (permission denied), but code is valid');
                // Don't fail the verification just because we can't update the timestamp
            }
            return true;
        }
...
function setCodeVerified(email) {
    ...
    // Get the current access code and store it
    firebase.firestore().collection('authorized_users').doc(email.toLowerCase())
        .get()
        .then(doc => {
            if (doc.exists) {
                const userData = doc.data();
                const code = userData.accessCode || userData.access_code;
                if (code) {
                    sessionStorage.setItem('verifiedCode_' + email, code.toString());
                }
            }
        })
        .catch(err => AUTH_LOG.error('Error storing verified code:', err));
...
function showCodeVerificationModal(email) {
    ...
    setTimeout(() => {
        ...
        verifyBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            AUTH_LOG.log('Verify button clicked (addEventListener)');
            handleVerification();
        });
       ...
       cancelBtn.onclick = async function(e) {
        ...
        cancelBtn.addEventListener('click', async function(e) {
            e.preventDefault();
            e.stopPropagation();
            AUTH_LOG.log('Cancel button clicked (addEventListener)');
            stopSessionMonitoring();
            await firebase.auth().signOut();
            modal.remove();
            toast.info('Signed Out', {
    ...
    try {
        ...
        if (result.user) {
            ...
            if (isCodeVerified(result.user.email)) {
                ...
                // Start session monitoring
                startSessionMonitoring(result.user.email);
    ...
    callbacks: {
        signInSuccessWithAuthResult: async function(authResult, redirectUrl) {
            ...
            if (isCodeVerified(user.email)) {
                ...
                // Start session monitoring
                startSessionMonitoring(user.email);
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (currentUser) {
        ...
        if (isCodeVerified(currentUser.email)) {
            ...
            startSessionMonitoring(currentUser.email);
