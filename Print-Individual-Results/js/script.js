...
const REPORT_CARD_CONFIG = {
    subjects: ['MATHS', 'ENG', 'KISW', 'SCI', 'C-A','SSTRE'],
    gradeScale: [
...
function extractGrade(gradeString) {
  ...
  async function loadStudentImage(studentName, grade) {
    return new Promise((resolve) => {
        ...
        img.src = `${trimmedGrade}/${studentName}.jpg`;
    ...
    doc.setFontSize(60);
    doc.text('FINAL COPY', pageWidth / 2, pageHeight / 2, {
        align: 'center',
        angle: 45
    });
...
async function generateBulkReportCards() {
    ...
    try {
        ...
        for (let i = 0; i < studentsToProcess.length; i++) {
            ...
            combinedDoc.text('FINAL COPY', pageWidth / 2, pageHeight / 2, {
                align: 'center',
                angle: 45
            });
