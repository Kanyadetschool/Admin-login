...
const HEARTBEAT_INTERVAL = 5 * 24 * 60 * 60 * 1000; // 5 days in milliseconds
const HEARTBEAT_STORAGE_KEY = 'supabase_last_heartbeat';

// Main heartbeat function
async function performHeartbeat() {
    try {
        console.log('🫀 Performing Supabase heartbeat...');
        
        // Method 1: Storage API ping (lightweight)
        const { data, error } = await supabase.storage
            .from(STORAGE_BUCKET)
            .list('', { limit: 1 });
        
        if (error) {
            console.warn('Heartbeat storage check failed:', error);
            // Try fallback method
        ...
        showToast('🫀 Supabase heartbeat completed', 'success');
...
// Fallback heartbeat method
async function performFallbackHeartbeat() {
    try {
        console.log('🔄 Trying fallback heartbeat...');
        
        // Create a tiny heartbeat file
        const heartbeatData = new Blob(['heartbeat'], { type: 'text/plain' });
        const timestamp = new Date().getTime();
        
        const { error } = await supabase.storage
            .from(STORAGE_BUCKET)
            .upload(`_heartbeat/ping_${timestamp}.txt`, heartbeatData, {
                cacheControl: '3600',
                upsert: true
            });
        ...
        if (!error) {
            ...
            // Clean up old heartbeat files (keep only last 3)
            cleanupHeartbeatFiles();
        }
...
async function cleanupHeartbeatFiles() {
    try {
        ...
        if (files && files.length > 3) {
            const filesToDelete = files
                ...
                .slice(3)
                .map(f => `_heartbeat/${f.name}`);
            
            await supabase.storage
                .from(STORAGE_BUCKET)
                .remove(filesToDelete);
            
            console.log(`🗑️ Cleaned up ${filesToDelete.length} old heartbeat files`);
        }
    } catch (error) {
...
function initializeHeartbeat() {
    ...
    if (!lastHeartbeat) {
        // First time - perform heartbeat after 3 seconds
        console.log('🫀 First heartbeat scheduled...');
        setTimeout(() => {
            performHeartbeat();
        }, 3000);
    } else {
        ...
        const daysAgo = Math.floor(timeSinceLastHeartbeat / (24 * 60 * 60 * 1000));
        
        if (timeSinceLastHeartbeat >= HEARTBEAT_INTERVAL) {
            // It's been 5+ days, perform heartbeat now
            console.log(`🫀 Heartbeat needed (${daysAgo} days ago)`);
            setTimeout(() => {
                performHeartbeat();
            }, 3000);
        } else {
            const nextHeartbeatDays = Math.ceil((HEARTBEAT_INTERVAL - timeSinceLastHeartbeat) / (24 * 60 * 60 * 1000));
            console.log(`⏰ Last heartbeat: ${daysAgo} days ago. Next in ${nextHeartbeatDays} days`);
        }
    }
    
    // Set up automatic heartbeat every 5 days
    setInterval(() => {
        performHeartbeat();
    }, HEARTBEAT_INTERVAL);
}
...
function updateHeartbeatStatus() {
    ...
    if (!lastHeartbeat) {
        statusEl.innerHTML = '🫀 <span class="text-gray-500">Never</span>';
        return;
    }
    ...
    let statusHTML = '';
    if (diffHours < 1) {
        statusHTML = '🫀 <span class="text-green-600 font-semibold">Just now</span>';
    } else if (diffHours < 24) {
        statusHTML = `🫀 <span class="text-green-600">${diffHours}h ago</span>`;
    } else if (diffDays === 1) {
        statusHTML = '🫀 <span class="text-green-600">Yesterday</span>';
    } else if (diffDays < 5) {
        statusHTML = `🫀 <span class="text-yellow-600">${diffDays} days ago</span>`;
    } else if (diffDays < 7) {
        statusHTML = `🫀 <span class="text-orange-600 font-semibold">${diffDays} days ago ⚠️</span>`;
    } else {
        statusHTML = `🫀 <span class="text-red-600 font-bold">${diffDays} days ago ❌</span>`;
    }
    
    statusEl.innerHTML = statusHTML;
}

// Manual heartbeat function (for button)
async function manualHeartbeat() {
    showToast('🫀 Pinging Supabase...', 'info');
    const success = await performHeartbeat();
    if (success) {
        updateHeartbeatStatus();
        showToast('✅ Supabase is now active!', 'success');
    } else {
        showToast('⚠️ Heartbeat completed with warnings', 'warning');
    }
}
...
onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
        userId = user.uid;
         ...
         // ✅ ADD THIS: Check heartbeat when user logs in
        setTimeout(() => {
            const lastHeartbeat = localStorage.getItem(HEARTBEAT_STORAGE_KEY);
            const now = Date.now();
            
            if (!lastHeartbeat || (now - new Date(lastHeartbeat).getTime() >= HEARTBEAT_INTERVAL)) {
                console.log('🫀 User login triggered heartbeat check');
                performHeartbeat();
            }
            updateHeartbeatStatus();
        }, 3000);
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('📚 Student Management App with Supabase Storage Initialized');
      ...
      // ✅ ADD THIS: Initialize heartbeat system on page load
    setTimeout(() => {
        initializeHeartbeat();
        updateHeartbeatStatus();
    }, 2000);
    
    // Update heartbeat status every minute
    setInterval(() => {
        updateHeartbeatStatus();
    }, 60000);
