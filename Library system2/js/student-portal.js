...
class StudentPortal {
    constructor() {
     ...
     // Add dashboard stats click handlers
...
setupSearchListener() {
    // Wait for DOM to be ready and set up search listener
...
async showFilteredBooks(filter) {
    try {
        // Switch to activeBooks view first
        this.switchView('activeBooks');

        // Load all books for the student
...
// Update the updateStudentInfo method to include student image rendering
// Update the updateStudentInfo method to include all student data in hover card
updateStudentInfo() {
    if (!this.studentData) {
    ...
    // Debug: Log available elements and student data
    ...
    if (elements.studentAvatar) {
        ...
        // Debug: Confirm hover card is added
    ...
    // Update other student info elements
    Object.entries(elements).forEach(([key, element]) => {
   ...
   // Replace or update your updateStatistics method with this:

updateStatistics(stats) {
    // Show loading state first
    this.showStatsLoading();
    
    // Simulate async operation or add actual delay if needed
    setTimeout(() => {
...
showStatsLoading() {
    ...
    // Show loading spinners on stat cards
    ...
    // Show loading for borrowing stats
    const borrowingStatsEl = document.getElementById('borrowingStats');
    if (borrowingStatsEl) {
        borrowingStatsEl.innerHTML = `
            <div class="text-center mb-2">
...
hideStatsLoading() {
    // Remove any loading classes or states if needed
    // This method can be expanded based on your specific loading implementation
    console.log('Stats loading completed');
}

// Alternative version with skeleton loading
...
//     // Show skeleton for borrowing stats
//     const borrowingStatsEl = document.getElementById('borrowingStats');
//     if (borrowingStatsEl) {
//         borrowingStatsEl.innerHTML = `
//             <div class="text-center mb-2">
...
// Add this helper method to make individual stats clickable
makeStatClickable(element, filter) {
    if (!element) return;
    
    // Find the parent container (card or stat container)
...
// Update the loadActiveBooks method to setup search after loading
async loadActiveBooks() {
    try {
        ...
        // Setup search listener after books are loaded
        const searchInput = document.querySelector('#activeBooksView input[type="text"]');
        if (searchInput) {
