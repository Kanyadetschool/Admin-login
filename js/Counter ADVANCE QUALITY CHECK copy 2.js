...
function showDataQualityModal(allIssues) {
    ...
    let pendingChanges = {}; // Track inline changes
 ...
 function createEditableInput(studentData, field, currentValue) {
    ...
    const modalHTML = `
        <div style="text-align: left;">
            ...
            <div style="margin-bottom: 15px; text-align: center; display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                ...
                <button id="saveInlineChanges" style="
                    ...
                    pointer-events: none;
                " disabled>
                    💾 Save Changes (<span id="inlineChangeCount">0</span>)
                </button>
            </div>
    ...
    Swal.fire({
        ...
        width: '800px',
        didOpen: () => {
            // Add event listener for fast edit button
...
async function saveInlineChanges() {
    ...
    Object.keys(pendingChanges).forEach(studentKey => {
        // Find the student data - it's nested in the 'student' property
        const studentData = allIssues.find(s => `${s.grade}-${s.name}` === studentKey);
        if (studentData && studentData.student) {
            // Update the nested student object with new values
            ...
            // Push the actual student object (not the wrapper)
        ...
        updateInlineChangeCount();
            ...
            function renderResults() {
                ...
                const htmlContent = filteredIssues.map((student, index) => {
                    ...
                    // Create editable fields for each issue
// Create editable fields for each issue
const editableFields = student.issues.map(issue => {
    // Get current value from the nested student object
    const currentValue = student.student[issue] || '';
    return `
        <div style="display: inline-block; margin: 4px; padding: 8px; 
             background: ${issueColors[issue] || '#dc3545'}15; 
             border: 1px solid ${issueColors[issue] || '#dc3545'}40; 
             border-radius: 6px; min-width: 150px; vertical-align: top;">
            <strong style="font-size: 11px; color: ${issueColors[issue] || '#dc3545'}; display: block; margin-bottom: 4px;">${issue}:</strong>
