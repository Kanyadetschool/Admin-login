...
function generateTableReport() {
    ...
    let table = $(`#${activeTabId}Table`).DataTable();
...
function generateTableRows(tabId, data) {
    const headerMap = generateTableHeaders(tabId).headerMap;
    ...
    return data.map(item => {
        switch(tabId) {
            case 'equipment':
                return `
                    <tr>
                        ...
                        <td>${item.value ? '$' + parseFloat(item.value).toLocaleString() : '$0'}</td>
                    </tr>
                `;
            case 'accounts':
                return `
                    <tr>
                        ...
                        <td>${item.category || ''}</td>
                        <td>${item.balance ? '$' + parseFloat(item.balance).toLocaleString() : '$0'}</td>
            ...
            case 'teachers':
                ...
                `;
            // Add similar cases for other table types
            default:
                const headers = headerMap[tabId] || [];
                return `
                    <tr>
                        ${headers.map(header => `<td>${item[header.field] || ''}</td>`).join('')}
                    </tr>
                `;
        }
    }).join('');
