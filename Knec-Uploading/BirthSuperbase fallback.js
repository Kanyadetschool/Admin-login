...
// Check if PDF exists in Supabase Storage - OPTIMIZED
async function checkSupabasePdfExists(studentName, studentGrade) {
    ...
    try {
        // Use single list call instead of multiple checks
        const { data, error } = await supabase.storage
            .from(STORAGE_BUCKET)
            .list(studentGrade, {
                limit: 100, // Check multiple at once
                search: studentName
            });
        
        if (error) throw error;
        
        // Check if any file matches (case-insensitive)
        const exists = data && data.some(file => 
            file.name.toLowerCase() === `${studentName}.pdf`.toLowerCase()
        );
...
window.openLocalPDF = async function(studentName, studentGrade) {
    const pdfContainer = document.getElementById('pdf-iframe'); // Reuse same element
    ...
    try {
        ...
        // Use embed for fastest loading - works on all devices
        pdfContainer.innerHTML = `
            <embed 
                src="${pdfUrl}" 
...
function startBackgroundPdfCheck() {
    ...
    // Group students by grade for efficient batch checking
    const gradeGroups = {};
    ...
    async function checkNextGrade() {
        ...
        try {
            ...
            if (!error && data) {
                ...
                // Check each student against the file list
                students.forEach(student => {
                    const pdfName = `${student.name}.pdf`.toLowerCase();
                    student.hasPdf = existingFiles.has(pdfName);
                    
                    // Update cache
                    const cacheKey = `${student.grade}/${student.name}`;
                    pdfExistenceCache.set(cacheKey, student.hasPdf);
                });
                
                console.log(`✓ Checked ${students.length} students in ${grade}`);
            }
        } catch (error) {
        ...
        if (currentGradeIndex % 3 === 0) {
            updateStatistics();
            renderStudents();
        }
        
        // Continue with next grade (small delay to prevent overwhelming)
        if (currentGradeIndex < grades.length && backgroundCheckRunning) {
            setTimeout(checkNextGrade, 200);
        } else {
            backgroundCheckRunning = false;
            updateStatistics();
            renderStudents();
            showToast('All PDFs checked!', 'success');
        }
    }
