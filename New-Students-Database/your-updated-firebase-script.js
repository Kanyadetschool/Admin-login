...
function refreshData() {
    ...
    localStorage.removeItem('pdfCheckCache'); // ✅ ADD THIS LINE
...
const gradeStatsContainer = getElementById('grade-stats-container');
...
window.openLocalPDF = function(studentName, studentGrade) {
    if (!pdfIframe || !pdfModalBackdrop) {
        const error = 'PDF modal elements not found. Please ensure pdfIframe and pdfModalBackdrop are defined.';
        console.error(error);
        alert(error);
    ...
    // const pdfPathUppercase = `https://kanyadet-school-admin.web.app/New-Students-Database/birth_certificates/${safeGrade}/${safeName}.PDF`;
    // const pdfPathLowercase = `https://kanyadet-school-admin.web.app/New-Students-Database/birth_certificates/${safeGrade}/${safeName}.pdf`;
   
    // Check if screen is mobile or tablet (width <= 1024px)
    const isMobileOrTablet = window.innerWidth <= 1024;
    
    let hasTriedLowercase = false;
    
    if (isMobileOrTablet) {
        // Use Google PDF Viewer for mobile and tablet
        const absolutePath = new URL(pdfPathUppercase, window.location.href).href;
        const googleViewerUrl = `https://docs.google.com/viewer?url=${encodeURIComponent(absolutePath)}&embedded=true`;
        pdfIframe.src = googleViewerUrl;
        
        // Error handler for mobile/tablet
        pdfIframe.onerror = function() {
            if (!hasTriedLowercase) {
                hasTriedLowercase = true;
                const absolutePathLower = new URL(pdfPathLowercase, window.location.href).href;
                pdfIframe.src = `https://docs.google.com/viewer?url=${encodeURIComponent(absolutePathLower)}&embedded=true`;
            } else {
                const error = `Failed to load PDF via Google Viewer.\nReason: PDF file not found or not publicly accessible.\nStudent: ${safeName}\nGrade: ${safeGrade}\nPaths tried:\n- ${pdfPathUppercase}\n- ${pdfPathLowercase}\n\nNote: Google Viewer requires publicly accessible URLs.`;
                console.error(error);
                alert(error);
                closePDFModal();
                throw new Error(error);
        ...
        // Additional check for Google Viewer load
        setTimeout(() => {
            if (pdfIframe.src && !pdfIframe.contentWindow) {
                const error = `Google PDF Viewer failed to load.\nPossible reasons:\n- PDF file is not publicly accessible\n- Network connectivity issues\n- CORS restrictions\n- Invalid PDF file`;
                console.error(error);
                alert(error);
            }
        }, 5000);
...
// PDF CHECK FUNCTIONS
// =============================================================================
// OPTIMIZED: Check both extensions in parallel - 2x faster!
async function checkLocalPdfExists(studentName, studentGrade) {
    ...
    try {
        ...
        // ✨ MAGIC: Check BOTH extensions at the same time!
        const [upperResponse, lowerResponse] = await Promise.allSettled([
            fetch(`${basePath}.PDF`, { method: 'HEAD' }),
            fetch(`${basePath}.pdf`, { method: 'HEAD' })
        ]);
        
        const exists = (upperResponse.status === 'fulfilled' && upperResponse.value.ok) ||
                      (lowerResponse.status === 'fulfilled' && lowerResponse.value.ok);
...
// OPTIMIZED: Check visible + preload next page
...
// REPLACE WITH THIS:
// REPLACE WITH THIS:
function getPdfCheckCache() {
    try {
        const cached = localStorage.getItem('pdfCheckCache');
        if (!cached) return {};
        ...
        if (parsed.timestamp && parsed.data) {
            ...
            if (Date.now() - parsed.timestamp > THIRTY_MINUTES) {
                console.log('🔄 Cache expired (5+ minutes old) - clearing');
                localStorage.removeItem('pdfCheckCache');
                return {};
            }
            
            console.log('📦 Using cached data (still fresh)');
            return parsed.data; // ✅ Return the data object
        }
        
        // ✅ OLD format detected - clear it and start fresh
        console.log('⚠️ Old cache format detected - clearing');
        localStorage.removeItem('pdfCheckCache');
        return {};
...
// REPLACE WITH THIS:
function savePdfCheckCache(cache) {
    try {
        const cacheData = {
            timestamp: Date.now(), // ✅ Current time in milliseconds
            data: cache           // ✅ The actual cache data
        };
        localStorage.setItem('pdfCheckCache', JSON.stringify(cacheData));
        console.log('💾 Cache saved with timestamp');
    } catch (error) {
        console.error('Failed to save PDF check cache:', error);
    }
}

function getCacheKey(studentName, grade) {
    return `${studentName}_${grade}`;
}
...
function startBackgroundPdfCheck() {
    ...
    const batchSize = 10; // Increased from 1 for better performance
    let currentIndex = 0;
    
    // Load cache once at the start
    const pdfCache = getPdfCheckCache();
    let cacheHits = 0;
    let cacheMisses = 0;
    
    // Apply cached results immediately
    allStudents.forEach(student => {
        const cacheKey = getCacheKey(student.name, student.grade);
        if (pdfCache[cacheKey] !== undefined) {
            student.hasPdf = pdfCache[cacheKey];
            cacheHits++;
        }
    });
    
    if (cacheHits > 0) {
        console.log(`📦 Loaded ${cacheHits} cached PDF checks`);
        updateStatistics();
        renderStudents();
    }
    ...
    async function checkNextBatch() {
        if (!backgroundCheckRunning || currentIndex >= allStudents.length) {
            backgroundCheckRunning = false;
            if (cacheMisses > 0) {
                showToast(`✅ Checked ${cacheMisses} new PDFs (${cacheHits} from cache)`, 'success');
            }
            return;
        }
        
        const batch = allStudents.slice(currentIndex, currentIndex + batchSize);
        // Only check students without cached results
        const unchecked = batch.filter(s => s.hasPdf === undefined);
        
        if (unchecked.length > 0) {
            try {
                await Promise.all(unchecked.map(async (student) => {
                    const exists = await checkLocalPdfExists(student.name, student.grade);
                    student.hasPdf = exists;
                    
                    // Update cache
                    const cacheKey = getCacheKey(student.name, student.grade);
                    pdfCache[cacheKey] = exists;
                    cacheMisses++;
                }));
                
                // Save cache periodically (every batch)
                savePdfCheckCache(pdfCache);
                
                updateStatistics();
                
                // Only re-render if batch overlaps with visible rows
        ...
        if (currentIndex < allStudents.length && backgroundCheckRunning) {
            setTimeout(checkNextBatch, 10); // Small delay to prevent blocking
        } else {
            backgroundCheckRunning = false;
        }
    }
...
// Add function to clear cache when needed (e.g., after uploading new PDFs)
function clearPdfCheckCache() {
    localStorage.removeItem('pdfCheckCache');
    allStudents.forEach(student => student.hasPdf = undefined);
    showToast('🔄 PDF cache cleared', 'info');
}

function forceRefreshPdfs() {
    showToast('🔄 Clearing cache...', 'info');
    localStorage.removeItem('pdfCheckCache');
    pdfExistenceCache.clear();
    allStudents.forEach(s => s.hasPdf = undefined);
    backgroundCheckRunning = false;
    setTimeout(() => {
        renderStudents();
        startBackgroundPdfCheck();
        showToast('✅ Checking PDFs now', 'success');
    }, 500);
}
window.forceRefreshPdfs = forceRefreshPdfs;
...
const filteredCountEl2 = getElement('filtered-count2');
    ...
    onValue(studentsRef, async (snapshot) => {
        ...
        if (snapshot.exists()) {
            ...
            allStudents = studentEntries.map(([assessmentNo, student]) => ({
                ...
                gender: student.Gender || 'Unknown', // ← ADD THIS LINE
...
function generateAdvancedAnalytics() {
    ...
    allStudents.forEach(s => {
        ...
        // Count gender - now we have it from the student object
...
// Export enhanced export functions to window
...
console.log('✅ Advanced Analytics module loaded with interactive charts and gender count');